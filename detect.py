...
def postprocess(outputs, B, num_classes, anchors, masks, conf_thresh, net_size):
    ...
    for i, x in enumerate(outputs):
        ...
        x[..., 0:2] = (x[..., 0:2].sigmoid() * 2 - 0.5 + grid) / gs
        x[..., 2:4] = (x[..., 2:4].sigmoid() * 2) ** 2 * anchor_grid / net_size
        x[..., 4:] = x[..., 4:].sigmoid()
...
if __name__ == '__main__':
    # load moel
    checkpoint_path = 'weights/yolov7_80.pth'
    B, C = 3, 20
    ...
    # postprocess
    results = postprocess(outputs, B, C, anchors, masks, thresh, net_size)
    ...
    # cv2.imwrite('assets/result1.jpg', img)
    cv2.imshow('demo', img)
    cv2.waitKey(0)